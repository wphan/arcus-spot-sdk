...
The hosted router presets (RH mainnet, testnet, Arbitrum) need no setup. To use the **local** preset instead, start a router with the venue keys it needs. Optional **API key** and **builder fee bps** fields map to `X-Api-Key` and `builderFeeBps` on `/price`, `/quote`, and `/submit`.
